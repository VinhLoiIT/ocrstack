...
class ICkptSaver:
    ...
    def get_metric_value(self, metrics: Dict[str, float]) -> float:
...
class MonitorCkpt(ICkptSaver):
    ...
    def get_metric_value(self, metrics: Dict[str, float]) -> float:
        return metrics[self.metric_monitor]
    ...
    def save(self, session_dir, trainer_state, eval_metrics):
        # type: (str, Dict, Dict[str, float]) -> None
        current_metric_value = eval_metrics[self.metric_monitor]
        self.__last_metric_value = current_metric_value

        filename = f'{self.metric_monitor}={current_metric_value:.04f}.pth'
